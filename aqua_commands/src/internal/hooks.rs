...
pub async fn dispatch_error(ctx: &Context, msg: &Message, error: DispatchError, _command_name: &str) {
	match error {
		...
		NotEnoughArguments { min, given } => {
			send_message(ctx, msg, format!("Got too few arguments. (Got {given} but expected >= {min}.)")).await;
		}
		TooManyArguments { max, given } => {
			send_message(ctx, msg, format!("Got too many arguments. (Got {given} but expected <= {max}.)")).await;
		}
		_ => {
